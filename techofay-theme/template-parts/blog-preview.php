...
<section class="relative py-24 sm:py-32 bg-[#F8FAF8] border-b border-[#E5E7EB] overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-16">
            <div>
                ...
                <h2 class="font-heading font-extrabold text-2xl sm:text-4xl text-[#111827] tracking-tight">
                    Latest from the <span class="text-[#16A34A]">Techofay Lab</span>
                </h2>
            </div>

            <a
                href="<?php echo esc_url( home_url( '/blog' ) ); ?>"
                class="inline-flex items-center gap-2 text-xs sm:text-sm font-semibold text-[#16A34A] hover:text-[#166534] transition-colors mt-4 sm:mt-0 group"
            >
            ...
            <?php if ( $blog_query->have_posts() ) : ?>
                <?php while ( $blog_query->have_posts() ) : $blog_query->the_post(); ?>
                    ...
                    <a
                        ...
                        class="group bg-white rounded-2xl overflow-hidden border border-[#E5E7EB] hover:border-[#16A34A] hover:shadow-[0_8px_24px_rgba(22,163,74,0.08)] transition-all flex flex-col justify-between"
                    >
                        <div>
                            <div class="relative aspect-video w-full overflow-hidden bg-[#F0FDF4]">
                                <img
                                    src="<?php echo esc_url( $thumb_url ); ?>"
                                    alt="<?php the_title_attribute(); ?>"
                                    class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                                />
                                <span class="absolute top-3 left-3 px-2.5 py-1 rounded-md text-[10px] font-mono uppercase bg-white/90 text-[#166534] border border-[#BBF7D0] backdrop-blur-md font-semibold">
                                    <?php echo esc_html( $cat_name ); ?>
                                </span>
                            </div>

                            <div class="p-6">
                                <div class="flex items-center gap-4 text-xs text-[#6B7280] mb-3">
                                    <span class="flex items-center gap-1.5">
                                        <svg class="w-3.5 h-3.5 text-[#16A34A]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                        <?php echo get_the_date( 'M d, Y' ); ?>
                                    </span>
                                    <span class="flex items-center gap-1.5">
                                        <svg class="w-3.5 h-3.5 text-[#16A34A]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                        5 min read
                                    </span>
                                </div>

                                <h3 class="font-heading font-bold text-base sm:text-lg text-[#111827] mb-3 group-hover:text-[#16A34A] transition-colors line-clamp-2">
                                    <?php the_title(); ?>
                                </h3>

                                <p class="text-xs sm:text-sm text-[#374151] leading-relaxed line-clamp-2">
                                    <?php echo wp_trim_words( get_the_excerpt(), 18 ); ?>
                                </p>
                            </div>
                        </div>

                        <div class="p-6 pt-0 border-t border-[#E5E7EB] flex items-center justify-between">
                            <div class="flex items-center gap-2.5">
                                <div class="w-7 h-7 rounded-full bg-[#DCFCE7] text-[#16A34A] font-bold text-xs flex items-center justify-center border border-[#16A34A]">
                                    <?php echo substr( get_the_author(), 0, 1 ); ?>
                                </div>
                                <span class="text-xs font-medium text-[#111827] truncate max-w-[120px]">
                                    <?php the_author(); ?>
                                </span>
                            </div>
                            <span class="text-xs font-semibold text-[#16A34A] flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                                Read Article &rarr;
                            </span>
                        </div>
                    </a>
                        ...
                        <div class="p-6 pt-0 border-t border-[#E5E7EB] flex items-center justify-between">
                            <div class="flex items-center gap-2.5">
                                <img
                                    ...
                                    class="w-7 h-7 rounded-full object-cover border border-[#16A34A]"
                                />
                                <span class="text-xs font-medium text-[#111827] truncate max-w-[120px]">
