...
<section class="relative z-20 -mt-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="rounded-2xl p-6 sm:p-8 bg-[#F0FDF4] border border-[#BBF7D0] shadow-sm relative overflow-hidden group">
        <!-- Top subtle green accent line -->
        <div class="absolute top-0 left-0 right-0 h-[2px] bg-[#16A34A] opacity-70"></div>

        <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 divide-y sm:divide-y-0 sm:divide-x divide-[#BBF7D0]">
            <?php foreach ( $stats as $idx => $stat ) : ?>
                <div class="pt-4 sm:pt-0 <?php echo $idx !== 0 ? 'sm:pl-6 lg:pl-8' : ''; ?> group/stat hover:translate-y-[-2px] transition-transform">
                    <div class="flex items-center gap-3 mb-2">
                        <div class="w-9 h-9 rounded-lg bg-[#DCFCE7] flex items-center justify-center text-[#16A34A] transition-all">
                            <?php echo $stat['icon']; ?>
                        </div>
                        <div class="font-heading font-extrabold text-2xl sm:text-3xl lg:text-4xl text-[#16A34A] flex items-center gap-1.5">
                            <span><?php echo esc_html( $stat['value'] ); ?></span>
                            <span class="w-1.5 h-1.5 rounded-full bg-[#16A34A] animate-ping"></span>
                        </div>
                    </div>
                    <div class="font-semibold text-xs sm:text-sm text-[#111827] group-hover/stat:text-[#16A34A] transition-colors">
