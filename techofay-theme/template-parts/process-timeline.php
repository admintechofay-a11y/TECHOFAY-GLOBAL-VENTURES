...
<section class="relative py-24 sm:py-32 bg-[#0A1628] overflow-hidden">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="text-center max-w-3xl mx-auto mb-16 sm:mb-20">
            ...
            <h2 class="font-heading font-extrabold text-2xl sm:text-4xl lg:text-5xl text-[#FFFFFF] tracking-tight mb-4">
                How We <span class="text-[#2B6EFA]">Work & Deliver</span>
            </h2>
            <p class="text-sm sm:text-base text-[#8B9AB5]">
        ...
        <div class="relative mb-10">
            ...
            <div class="hidden lg:block absolute top-1/2 left-4 right-4 h-0.5 bg-[#E5E7EB] -translate-y-1/2 pointer-events-none z-0">
                <div id="timeline-progress-bar" class="h-full bg-[#2B6EFA] transition-all duration-500 w-0"></div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 relative z-10">
                <?php foreach ( $steps as $idx => $s ) : ?>
                    <button
                        type="button"
                        class="timeline-step-tab text-left p-4 rounded-xl border transition-all duration-300 flex flex-col justify-between cursor-pointer hover:scale-[1.02] <?php echo $idx === 0 ? 'bg-[rgba(255,255,255,0.05)] border-[#2B6EFA] shadow-[0_4px_12px_rgba(22,163,74,0.12)]' : 'bg-[#0A1628] border-[rgba(43,110,250,0.2)] hover:border-[rgba(0,212,255,0.3)]'; ?>"
                        data-step-index="<?php echo $idx; ?>"
                    >
                        <div class="flex items-center justify-between w-full mb-3">
                            <span class="step-label font-mono font-bold text-xs flex items-center gap-1.5 <?php echo $idx === 0 ? 'text-[#2B6EFA]' : 'text-[#8B9AB5]'; ?>">
                                <?php if ( $idx === 0 ) : ?><span class="ping-dot w-1.5 h-1.5 rounded-full bg-[#2B6EFA] animate-ping"></span><?php endif; ?>
                                STEP <?php echo esc_html( $s['step'] ); ?>
                            </span>
                            <span class="step-icon transition-transform <?php echo $idx === 0 ? 'text-[#2B6EFA] scale-110' : 'text-[#8B9AB5]'; ?>">
                                <?php echo $s['icon']; ?>
                            </span>
                        </div>
                        <div>
                            <div class="step-title font-heading font-bold text-sm <?php echo $idx === 0 ? 'text-[#FFFFFF]' : 'text-[#c4d7f5]'; ?>">
                                <?php echo esc_html( $s['title'] ); ?>
                            </div>
                            <div class="text-[10px] text-[#8B9AB5] truncate mt-0.5">
        ...
        <div class="bg-[#0A1628] rounded-2xl p-6 sm:p-10 border border-[rgba(43,110,250,0.2)] shadow-[0_8px_24px_rgba(22,163,74,0.06)]">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                
                <div class="lg:col-span-7 space-y-4">
                    <div class="inline-flex items-center gap-2 text-xs font-mono text-[#2B6EFA] font-semibold">
                        <span id="detail-phase-header">PHASE 01 OF 06</span>
                        <span>&bull;</span>
                        <span id="detail-phase-duration">TIMELINE: Week 1 - 2</span>
                    </div>
                    <h3 class="font-heading font-bold text-2xl sm:text-3xl text-[#FFFFFF]">
                        <span id="detail-title">Discovery</span>: <span id="detail-subtitle" class="text-[#2B6EFA]">Technical Audit & Growth Strategy</span>
                    </h3>
                    <p id="detail-focus" class="text-sm text-[#c4d7f5] leading-relaxed">
                        Deep exploration of existing systems, lead generation bottlenecks, and strategic enterprise objectives.
                    </p>

                    <div class="pt-2">
                        <div class="text-xs font-heading font-semibold uppercase text-[#FFFFFF] tracking-wider mb-3">
                            Core Phase Deliverables:
                        </div>
                        <div id="detail-deliverables" class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                            <div class="flex items-center gap-2 text-xs text-[#c4d7f5]">
                                <svg class="w-3.5 h-3.5 text-[#2B6EFA] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                ...
                <div class="lg:col-span-5 flex flex-col items-center justify-center p-8 rounded-xl bg-[#070E24] border border-[rgba(43,110,250,0.2)] text-center">
                    <div class="w-16 h-16 rounded-full bg-[rgba(43,110,250,0.2)] flex items-center justify-center text-[#2B6EFA] mb-4">
                        <svg class="w-8 h-8 text-[#2B6EFA]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                    </div>
                    <div class="font-heading font-bold text-lg text-[#FFFFFF] mb-1">
                        Zero Friction Handoff
                    </div>
                    <p class="text-xs text-[#8B9AB5] max-w-xs">
    ...
    tabs.forEach((tab, index) => {
        tab.addEventListener('click', function() {
            ...
            tabs.forEach((t, i) => {
                ...
                if (i === index) {
                    t.className = 'timeline-step-tab text-left p-4 rounded-xl border transition-all duration-300 flex flex-col justify-between cursor-pointer hover:scale-[1.02] bg-[rgba(255,255,255,0.05)] border-[#2B6EFA] shadow-[0_4px_12px_rgba(22,163,74,0.12)]';
                    if (label) {
                        label.className = 'step-label font-mono font-bold text-xs flex items-center gap-1.5 text-[#2B6EFA]';
                        label.insertAdjacentHTML('afterbegin', '<span class="ping-dot w-1.5 h-1.5 rounded-full bg-[#2B6EFA] animate-ping"></span>');
                    }
                    if (icon) icon.className = 'step-icon transition-transform text-[#2B6EFA] scale-110';
                    if (title) title.className = 'step-title font-heading font-bold text-sm text-[#FFFFFF]';
                } else if (i < index) {
                    t.className = 'timeline-step-tab text-left p-4 rounded-xl border transition-all duration-300 flex flex-col justify-between cursor-pointer hover:scale-[1.02] bg-[#070E24] border-[rgba(0,212,255,0.3)]';
                    if (label) label.className = 'step-label font-mono font-bold text-xs flex items-center gap-1.5 text-[#8B9AB5]';
                    if (icon) icon.className = 'step-icon transition-transform text-[#8B9AB5]';
                    if (title) title.className = 'step-title font-heading font-bold text-sm text-[#c4d7f5]';
                } else {
                    t.className = 'timeline-step-tab text-left p-4 rounded-xl border transition-all duration-300 flex flex-col justify-between cursor-pointer hover:scale-[1.02] bg-[#0A1628] border-[rgba(43,110,250,0.2)] hover:border-[rgba(0,212,255,0.3)]';
                    if (label) label.className = 'step-label font-mono font-bold text-xs flex items-center gap-1.5 text-[#8B9AB5]';
                    if (icon) icon.className = 'step-icon transition-transform text-[#8B9AB5]';
                    if (title) title.className = 'step-title font-heading font-bold text-sm text-[#c4d7f5]';
                }
