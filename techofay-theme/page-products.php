    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-12 relative">
        <div class="text-center max-w-3xl mx-auto space-y-4">
            ...
            <h1 class="font-heading font-extrabold text-3xl sm:text-5xl lg:text-6xl text-[#FFFFFF] tracking-tight">
                Enterprise <span class="text-[#2B6EFA]">Software Suites</span>
            </h1>
        ...
        <div class="mt-10 flex flex-col md:flex-row items-center justify-between gap-4 bg-[#0A1628] p-3 rounded-2xl border border-[rgba(43,110,250,0.2)] shadow-sm">
            ...
            <div class="flex flex-wrap items-center gap-2 w-full md:w-auto" id="product-filter-tabs">
                <button type="button" data-filter="all" class="prod-filter-btn px-4 py-2 rounded-lg text-xs font-semibold transition-all cursor-pointer bg-[#2B6EFA] text-[#1c1400] font-semibold shadow-sm">
                    All Products (5)
                </button>
                <button type="button" data-filter="Enterprise ERP" class="prod-filter-btn px-3 py-2 rounded-lg text-xs font-medium transition-all cursor-pointer text-[#c4d7f5] hover:text-[#2B6EFA] bg-[#070E24] border border-[rgba(43,110,250,0.2)]">
                    Enterprise ERP
                </button>
                <button type="button" data-filter="Enterprise Hardware" class="prod-filter-btn px-3 py-2 rounded-lg text-xs font-medium transition-all cursor-pointer text-[#c4d7f5] hover:text-[#2B6EFA] bg-[#070E24] border border-[rgba(43,110,250,0.2)]">
                    Smart NFC
                </button>
                <button type="button" data-filter="Applied AI" class="prod-filter-btn px-3 py-2 rounded-lg text-xs font-medium transition-all cursor-pointer text-[#c4d7f5] hover:text-[#2B6EFA] bg-[#070E24] border border-[rgba(43,110,250,0.2)]">
                    AI Chatbot
                </button>
                <button type="button" data-filter="Healthcare" class="prod-filter-btn px-3 py-2 rounded-lg text-xs font-medium transition-all cursor-pointer text-[#c4d7f5] hover:text-[#2B6EFA] bg-[#070E24] border border-[rgba(43,110,250,0.2)]">
                    Healthcare
                </button>
                <button type="button" data-filter="Education" class="prod-filter-btn px-3 py-2 rounded-lg text-xs font-medium transition-all cursor-pointer text-[#c4d7f5] hover:text-[#2B6EFA] bg-[#070E24] border border-[rgba(43,110,250,0.2)]">
                    Education
                </button>
            </div>
                ...
                <?php while ( $products_query->have_posts() ) : $products_query->the_post(); ?>
                    ...
                    <article class="product-card bg-[#0A1628] rounded-2xl p-8 flex flex-col justify-between border border-[rgba(43,110,250,0.2)] hover:border-[#2B6EFA] shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1.5" data-title="<?php the_title_attribute(); ?>" data-desc="<?php echo esc_attr( get_the_excerpt() ); ?>" data-category="<?php echo esc_attr( $category ); ?>">
                        <div>
                            ...
                            <!-- Pricing -->
                            <div class="mb-6 p-4 rounded-xl bg-[#070E24] border border-[rgba(43,110,250,0.2)]">
                                <div class="text-[10px] uppercase font-mono tracking-wider text-[#8B9AB5]">Starting Price</div>
                                <div class="flex items-baseline gap-1 mt-1">
                                    <span class="text-2xl font-extrabold text-[#FFFFFF]"><?php echo esc_html( $price ); ?></span>
                                    <span class="text-xs text-[#8B9AB5]"><?php echo esc_html( $billing ); ?></span>
                                </div>
                            </div>

                            <!-- Features List -->
                            <div class="space-y-2.5 mb-8">
                                <div class="text-xs font-bold uppercase tracking-wider text-[#FFFFFF]">Key Capabilities</div>
                                <?php foreach ( array_slice( $features, 0, 4 ) as $feat ) : ?>
                                    <div class="flex items-start gap-2 text-xs text-[#c4d7f5]">
                        ...
                        <div class="pt-5 border-t border-[rgba(43,110,250,0.2)] flex items-center justify-between gap-3">
                            <a href="<?php the_permalink(); ?>" class="flex-1 py-3 rounded-xl text-xs font-bold text-center text-[#1c1400] font-semibold bg-[#2B6EFA] hover:bg-[#1E50C8] shadow-sm transition-all">
                                Request Live Demo
                            </a>
                            <a href="<?php the_permalink(); ?>" class="px-4 py-3 rounded-xl text-xs font-bold text-[#2B6EFA] bg-[#0A1628] border border-[#2B6EFA] hover:bg-[rgba(255,255,255,0.05)] transition-all">
                                Specs &rarr;
                            </a>
                        </div>
                    </article>
                    ...
                    <article class="product-card bg-[#0A1628] rounded-2xl p-8 flex flex-col justify-between border border-[rgba(43,110,250,0.2)] hover:border-[#2B6EFA] shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1.5" data-title="<?php echo esc_attr( $prod['name'] ); ?>" data-desc="<?php echo esc_attr( $prod['desc'] ); ?>" data-category="<?php echo esc_attr( $prod['category'] ); ?>">
                        <div>
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <span class="text-xs font-mono font-bold text-[#2B6EFA]"><?php echo esc_html( $prod['metric'] ); ?></span>
                            </div>
                            ...
                            <div class="mb-6 p-4 rounded-xl bg-[#070E24] border border-[rgba(43,110,250,0.2)]">
                                <div class="text-[10px] uppercase font-mono tracking-wider text-[#8B9AB5]">Starting Price</div>
                                <div class="flex items-baseline gap-1 mt-1">
                                    <span class="text-2xl font-extrabold text-[#FFFFFF]"><?php echo esc_html( $prod['price'] ); ?></span>
                                    <span class="text-xs text-[#8B9AB5]"><?php echo esc_html( $prod['billing'] ); ?></span>
                                </div>
                            </div>
                        ...
                        <div class="pt-5 border-t border-[rgba(43,110,250,0.2)] flex items-center justify-between gap-3">
                            <a href="<?php echo esc_url( home_url( '/contact' ) ); ?>" class="flex-1 py-3 rounded-xl text-xs font-bold text-center text-[#1c1400] font-semibold bg-[#2B6EFA] hover:bg-[#1E50C8] shadow-sm transition-all">
                                Request Live Demo
                            </a>
                            <a href="<?php echo esc_url( home_url( '/product/' . $prod['slug'] ) ); ?>" class="px-4 py-3 rounded-xl text-xs font-bold text-[#2B6EFA] bg-[#0A1628] border border-[#2B6EFA] hover:bg-[rgba(255,255,255,0.05)] transition-all">
                                Specs &rarr;
                            </a>
                        </div>
                    </article>
