...
export default function RoiCalculator({ onSelectProductForDemo }) {
  ...
  return (
    <div className="w-full bg-[#0A1628] rounded-3xl border border-[rgba(43,110,250,0.2)] p-6 sm:p-10 shadow-[0_12px_40px_rgba(43,110,250,0.08)] relative overflow-hidden">
      {/* Header */}
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 pb-8 border-b border-[rgba(43,110,250,0.2)]">
        <div>
          <div className="flex items-center gap-2 text-[#2B6EFA] font-mono text-xs uppercase tracking-wider mb-1.5 bg-[#2B6EFA]/10 border border-[#2B6EFA]/30 px-3 py-1 rounded-full w-fit">
            <Calculator className="w-4 h-4 text-[#2B6EFA]" />
            <span>Interactive ROI & Cost Model (INR ₹)</span>
          </div>
          <h3 className="font-heading font-extrabold text-xl sm:text-2xl text-[#FFFFFF]">
            Enterprise Value & Savings Forecaster
          </h3>
          <p className="text-xs sm:text-sm text-[#c4d7f5] mt-1 max-w-2xl">
        ...
        <div className="px-4 py-2 rounded-xl bg-[#050B1F] border border-[rgba(43,110,250,0.2)] flex items-center gap-3">
          <div className="w-2.5 h-2.5 rounded-full bg-[#2B6EFA] animate-pulse" />
          <span className="text-xs text-[#c4d7f5]">
            Currency: <strong className="text-[#2B6EFA]">Indian Rupee (₹ INR)</strong>
          </span>
        </div>
      </div>
      ...
      <div className="relative z-10 my-8">
        <label className="block text-xs font-semibold text-[#c4d7f5] uppercase tracking-wider mb-3">
            ...
            return (
              <button
                ...
                className={`p-3.5 rounded-xl border text-left transition-all relative overflow-hidden flex flex-col gap-2 cursor-pointer ${
                  isSelected
                    ? 'bg-[#2B6EFA]/15 border-2 border-[#2B6EFA] text-[#FFFFFF] shadow-[0_0_15px_rgba(43,110,250,0.2)]'
                    : 'bg-[#050B1F] border-[rgba(43,110,250,0.2)] text-[#c4d7f5] hover:border-[#2B6EFA]/50'
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isSelected ? 'bg-[#2B6EFA]/20 text-[#2B6EFA]' : 'bg-[#0A1628] text-[#8B9AB5]'}`}>
                    <Icon className="w-4 h-4" />
                  </div>
                  {isSelected && (
                    <CheckCircle2 className="w-4 h-4 text-[#2B6EFA]" />
                  )}
                </div>
                <span className="font-heading font-bold text-xs leading-snug line-clamp-2 text-[#FFFFFF]">
                  {prod.name}
                </span>
                <span className="text-[10px] font-mono text-[#2B6EFA] font-semibold">
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        <div className="lg:col-span-6 space-y-6 bg-[#050B1F] p-6 rounded-2xl border border-[rgba(43,110,250,0.2)]">
          <h4 className="font-heading font-bold text-sm text-[#FFFFFF] flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center justify-between text-xs mb-2">
              <span className="text-[#c4d7f5] font-medium">{activeProduct.unitLabel}</span>
              <span className="font-heading font-bold text-[#2B6EFA] text-sm px-2.5 py-0.5 rounded-lg bg-[#2B6EFA]/10 border border-[#2B6EFA]/30">
            ...
            <input
              ...
              className="w-full h-2 bg-[#0A1628] rounded-lg appearance-none cursor-pointer accent-[#2B6EFA]"
            />
            <div className="flex justify-between text-[10px] text-[#8B9AB5] mt-1 font-mono">
          ...
          <div>
            <div className="flex items-center justify-between text-xs mb-2">
              <span className="text-[#c4d7f5] font-medium">Avg. Hourly Cost per Staff Member</span>
              <span className="font-heading font-bold text-[#2B6EFA] text-sm px-2.5 py-0.5 rounded-lg bg-[#2B6EFA]/10 border border-[#2B6EFA]/30">
                ₹{hourlyRate}/hr
              </span>
            </div>
          ...
          <div className="p-3 rounded-xl bg-[#0A1628] border border-[rgba(43,110,250,0.2)] flex items-center gap-3 text-[11px] text-[#c4d7f5]">
            <ShieldCheck className="w-4 h-4 text-[#2B6EFA] shrink-0" />
            <span>
        ...
        <div className="lg:col-span-6 space-y-4">
          <div className="p-6 rounded-2xl bg-[rgba(255,255,255,0.05)] border border-[rgba(0,212,255,0.3)] shadow-xs">
            <div className="grid grid-cols-2 gap-4 pb-4 border-b border-[rgba(43,110,250,0.2)]">
              <div>
                <span className="text-[10px] font-mono uppercase text-[#8B9AB5] block">
                  Est. Monthly Investment
                </span>
                <span className="font-heading font-extrabold text-xl text-[#FFFFFF]">
              ...
              <div>
                <span className="text-[10px] font-mono uppercase text-[#2B6EFA] font-semibold block">
                  Monthly Labor Recovered
                </span>
                <span className="font-heading font-extrabold text-xl text-[#2B6EFA]">
            ...
            <div className="pt-4 grid grid-cols-2 gap-4">
              <div>
                ...
                <span className="font-heading font-black text-2xl text-[#2B6EFA]">
              ...
              <div className="flex flex-col justify-center items-start pl-2">
                ...
                <span className="font-heading font-black text-3xl text-[#00D4FF]">
          ...
          <button
            ...
            className="w-full py-4 px-6 rounded-xl bg-[#2B6EFA] hover:bg-[#1E50C8] text-white font-semibold hover:text-white font-heading font-bold text-xs uppercase tracking-wider shadow-[0_0_20px_rgba(43,110,250,0.3)] transition-all flex items-center justify-center gap-2 group cursor-pointer"
          >
            <Sparkles className="w-4 h-4 text-white font-semibold group-hover:text-white" />
