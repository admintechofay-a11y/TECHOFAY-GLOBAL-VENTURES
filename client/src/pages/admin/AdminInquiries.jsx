...
export default function AdminInquiries() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-[#1A1A1A] border border-[rgba(245,158,11,0.2)] text-[10px] font-mono">
              <span className={`w-1.5 h-1.5 rounded-full ${isConnected ? 'bg-[#F59E0B] animate-pulse' : 'bg-amber-600'}`} />
              <span className="text-[#FDE68A]/70">{isConnected ? 'Live WebSockets' : 'Connecting'}</span>
              <span className="text-[#F59E0B]">({inquiries.length})</span>
            </div>
          </div>
          <h1 className="font-heading font-extrabold text-2xl text-[#FFFBEB] mt-1">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2.5 rounded-xl text-xs font-semibold text-[#FDE68A]/70 hover:text-[#FFFBEB] bg-[#1A1A1A] border border-[rgba(245,158,11,0.2)] hover:border-[#F59E0B] transition-all flex items-center gap-1.5 cursor-pointer"
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl text-xs font-semibold text-[#1c1400] bg-[#F59E0B] hover:bg-[#B45309] shadow-[0_0_12px_rgba(245,158,11,0.2)] transition-all flex items-center gap-2 self-start sm:self-auto cursor-pointer"
          >
      ...
      <div className="bg-[#1A1A1A] p-4 rounded-2xl border border-[rgba(245,158,11,0.15)] flex flex-col md:flex-row items-center justify-between gap-4">
        <form onSubmit={handleSearchSubmit} className="relative w-full md:w-80">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#FDE68A]/50" />
          <input
            type="text"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search name, company, email..."
            className="w-full pl-10 pr-4 py-2 rounded-xl bg-[#111111] border border-[rgba(245,158,11,0.2)] text-xs text-[#FFFBEB] placeholder:text-[#FDE68A]/40 focus:outline-none focus:border-[#F59E0B]"
          />
        </form>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="px-3 py-2 rounded-xl bg-[#111111] border border-[rgba(245,158,11,0.2)] text-xs text-[#FFFBEB] focus:outline-none focus:border-[#F59E0B]"
          >
          ...
          {(statusFilter !== 'All' || serviceFilter !== 'All' || search) && (
            <button
              ...
              className="text-[11px] text-[#F59E0B] hover:underline px-2 py-1 cursor-pointer"
            >
              Reset Filters
            </button>
          )}
                  ...
                  <tr key={inq._id} className="hover:bg-white/5 transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5">
                        ...
                        <div className="font-semibold text-[#FFFBEB] text-xs">
                      ...
                      {inq.phone && <div className="text-[10px] text-[#FDE68A]/40 font-mono">{inq.phone}</div>}
                    </td>

                    <td className="px-5 py-4 font-medium text-[#FDE68A]">
                      <span className="px-2 py-0.5 rounded-md bg-[#111111] border border-[rgba(245,158,11,0.15)] text-[11px]">
                        {inq.service || 'General Consultation'}
                      </span>
                    </td>

                    <td className="px-5 py-4">
                      <div className="text-[#F59E0B] font-mono font-medium">
                        {inq.budget || 'Custom / Flexible'}
                      </div>
                      <div className="text-[10px] text-[#FDE68A]/60">
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-white/5 hover:bg-[rgba(245,158,11,0.15)] text-[#F59E0B] hover:text-[#FFFBEB] transition-colors cursor-pointer"
        ...
        {selectedInquiry && (
          <div className="space-y-4 text-xs text-[#FDE68A]">
            <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-[#111111] border border-[rgba(245,158,11,0.15)]">
              <div>
                <span className="text-[10px] uppercase font-mono block text-[#FDE68A]/50">Email Address</span>
                <a href={`mailto:${selectedInquiry.email}`} className="text-[#FFFBEB] font-medium hover:text-[#F59E0B]">
                  {selectedInquiry.email || 'N/A'}
                </a>
              </div>
              <div>
                <span className="text-[10px] uppercase font-mono block text-[#FDE68A]/50">Phone / WhatsApp</span>
                <span className="text-[#FFFBEB] font-medium">{selectedInquiry.phone || 'N/A'}</span>
              </div>
              <div>
                <span className="text-[10px] uppercase font-mono block text-[#FDE68A]/50">Company</span>
                <span className="text-[#FFFBEB] font-medium">{selectedInquiry.companyName || selectedInquiry.company || 'Individual'}</span>
              </div>
              <div>
                <span className="text-[10px] uppercase font-mono block text-[#FDE68A]/50">Service Vertical</span>
                <span className="text-[#F59E0B] font-medium">{selectedInquiry.service || 'General Consultation'}</span>
              </div>
              <div>
                <span className="text-[10px] uppercase font-mono block text-[#FDE68A]/50">Budget</span>
                <span className="text-[#FFFBEB] font-mono">{selectedInquiry.budget || 'Custom / Flexible'}</span>
              </div>
              <div>
                <span className="text-[10px] uppercase font-mono block text-[#FDE68A]/50">Timeline</span>
                <span className="text-[#FFFBEB]">{selectedInquiry.timeline || '1 - 3 Months'}</span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-[#111111] border border-[rgba(245,158,11,0.15)]">
              ...
              <p className="text-[#FFFBEB] text-xs leading-relaxed whitespace-pre-wrap">
            ...
            {selectedInquiry.attachment && (
              <div className="p-3 rounded-xl bg-[#111111] border border-[rgba(245,158,11,0.2)] flex items-center justify-between">
                <div className="flex items-center gap-2 text-[#FFFBEB] text-xs">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-xs text-[#F59E0B] hover:text-[#FCD34D] hover:underline"
                >
                  Download / View &rarr;
                </a>
              </div>
            ...
            {selectedInquiry.replies && selectedInquiry.replies.length > 0 && (
              <div className="space-y-2 pt-2">
                ...
                {selectedInquiry.replies.map((rep, idx) => (
                  <div key={idx} className="p-3 rounded-lg bg-[#111111] border border-[rgba(245,158,11,0.1)] text-[11px] text-[#FDE68A]">
                    <div className="flex justify-between text-[10px] text-[#FDE68A]/50 mb-1">
            ...
            <div className="pt-4 border-t border-[rgba(245,158,11,0.15)] flex justify-end gap-3">
              <button
                onClick={() => {
                  setReplyModalInquiry(selectedInquiry);
                  setSelectedInquiry(null);
                }}
                className="px-4 py-2 rounded-lg text-xs font-semibold text-[#1c1400] bg-[#F59E0B] hover:bg-[#B45309] flex items-center gap-2 cursor-pointer transition-colors"
              >
      ...
      >
        <form onSubmit={handleSendReply} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-[#FDE68A] mb-1">Reply Message</label>
            <textarea
              required
              rows={5}
              value={replyMessage}
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-[#111111] border border-[rgba(245,158,11,0.2)] text-[#FFFBEB] text-xs focus:outline-none focus:border-[#F59E0B]"
            />
          </div>
          ...
          <div className="pt-2 flex justify-end gap-3">
            ...
            <button
              ...
              className="px-5 py-2 rounded-lg text-xs font-semibold text-[#1c1400] bg-[#F59E0B] hover:bg-[#B45309] flex items-center gap-2 cursor-pointer transition-colors"
            >
